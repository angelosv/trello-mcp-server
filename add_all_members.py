#!/usr/bin/env python3
"""
Script para agregar ambos Miguels y Angelo a todas las tarjetas sin remover a nadie.
"""
...
ANGELO_ID = "5c8ba472ce95df5cb68edf00"     # angelosv1
...
async def add_member_to_card(client: httpx.AsyncClient, card_id: str, member_id: str):
    ...
    try:
        ...
        if e.response.status_code == 400:
            # Ya está asignado o error, pero no fallar
            print(f"⚠️  Member {member_id} ya está asignado o error en tarjeta {card_id}")
        else:
...
async def process_cards():
    """Procesa todas las tarjetas agregando ambos Miguels y Angelo."""
    async with httpx.AsyncClient() as client:
        ...
        members_to_add = [MIGUEL_1_ID, MIGUEL_2_ID, ANGELO_ID]
        ...
        for card_id in CARD_IDS:
            ...
            for member_id in members_to_add:
                if member_id == MIGUEL_1_ID:
                    if await add_member_to_card(client, card_id, member_id):
                        success_miguel1 += 1
                elif member_id == MIGUEL_2_ID:
                    if await add_member_to_card(client, card_id, member_id):
                        success_miguel2 += 1
                elif member_id == ANGELO_ID:
                    if await add_member_to_card(client, card_id, member_id):
                        success_angelo += 1
        ...
        print(f"\n✅ Completado!")
        print(f"   Miguel 1 agregado: {success_miguel1}/{total}")
        print(f"   Miguel 2 agregado: {success_miguel2}/{total}")
        print(f"   Angelo agregado: {success_angelo}/{total}")
